...
class TestCustomOpDevice(CustomOpTestCaseBase):
    ...
    def test_impl_separate(self, device):
        ...
        @custom_ops.impl(f"{self.test_ns}::foo", device_types=device)
        def foo_cuda(x):
            return x.cos()
    ...
    def test_impl_device_accelerator(self, device):
        ...
        self._test_impl_device("foo5", [device], device)
        self._test_impl_device("foo6", ["cpu", device], device)
...
class TestCustomOpAPIDevice(TestCase):
    ...
    def test_multi_types(self, device):
        @torch.library.custom_op(
            "_torch_testing::f", mutates_args=(), device_types=("cpu", device)
        )
        def f(x: Tensor) -> Tensor:
            x_np = x.cpu().numpy()
            out_np = np.sin(x_np)
